...
- In `tonys-workbench-ui`, Angular code lives in `client/` and BFF code lives in `server/`.
...
- Java 25 LTS Spring Boot services in `tonys-workbench-services` remain the domain and system-of-record services for business use cases, transactions, persistence, integrations, events, background work, and durable authorization enforcement.
