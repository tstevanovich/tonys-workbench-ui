...
Tony's Workbench is both a real application and a planning model for future work. The UI repository should help define standards that can guide this project and sibling repositories the owner may work with later. Architecture, documentation, components, Node.js server code, optional Java services, database assets, deployment assets, and quality gates should be planned before implementation grows around them.
...
- Node.js server package and module structure in `tonys-workbench-ui/server`.
