...
export const ModernTechStackArticle: DocumentationArticle = {
  ...
  sections: [
    {
      heading: 'Purpose',
      body: 'This page is the stack inventory. It lists the chosen technologies, the role each technology plays, and a short reason for the choice. Architecture rules, folder placement, error handling patterns, and workflow standards live in the other documentation pages.'
    },
    {
...
| Persistent docs storage | SQL Server-backed markdown | Storage model for editable, permissioned, reviewed, versioned, and user-authored documentation. |
`
    },
    {
...
| Migration format | Liquibase changelog files | Versioned database changes stored with the application source. |
...
| Java artifact publishing | Maven publication | Backend services publish executable Spring Boot jar artifacts through Maven-compatible publication metadata. |
